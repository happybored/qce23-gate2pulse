...
import datetime
import sys
from colib.rzx_compiler import rzx_compiler
...
class QiskitProcessor(object):
    def __init__(self,
                 ...
                 hub='ibm-q-education',
                 group='george-mason-uni-1',
                 project='hardware-acceler',
                 layout_method=None,
                 routing_method=None
                 ):
    ...
    def preprocess_parameterized(self,
        ...
        circ = circ_parameterized.compose(circ_fixed)
        circ = circ.compose(circ_measurement) 
    ...
    def preprocess_parameterized_pulse(self,
                                 q_device,
                                 q_layer_parameterized,
                                 q_layer_fixed,
                                 q_layer_measure,
        ...
        for inputs_single in x:
            ...
            for k, input_single in enumerate(inputs_single):
                binds[params[k]] = input_single.item()
            binds_all.append(binds)
        for bind in binds_all:
            temp_circ = circ.assign_parameters(bind)
            # print(temp_circ)
            sche =  rzx_compiler(self.backend,temp_circ)
    ...
    def process_parameterized(self, q_device: tq.QuantumDevice,
        ...
        if self.use_pulse:
            schedules = self.preprocess_parameterized_pulse(q_device, q_layer_parameterized, q_layer_fixed,
            q_layer_measure, x)

            chunk_size = self.max_experiments
            split_schedules = [schedules[i:i + chunk_size] for i in range(0, len(schedules), chunk_size)]
            ...
            measured_qiskit = get_expectations_from_counts(
                sum_counts, n_wires=q_device.n_wires)
