...
def main(backend_name):
    ...
    qct = transpile(circ, backend)
    sche1 = qiskit.schedule(qct, backend)
    print('gate-based ','duration:',sche1.duration,)
    
    sche2 = rzx_compiler(backend,circ)
    print("pulse-based ",'duration:',sche2.duration) 


    print(f"\nTest with Qiskit Real Backend")
    processor_simulation = QiskitProcessor(use_real_qc=True,backend_name = backend_name,hub='ibm-q-ornl', group='ornl', project='csc517')
    model.set_qiskit_processor(processor_simulation)
    valid(dataflow, 'test', model, device, qiskit=True)  

    print(f"\nTest with Pulse Schedule")
    processor = QiskitProcessor(use_real_qc=True,backend_name=backend_name,hub='ibm-q-ornl', group='ornl', project='csc517',use_pulse= True)
...
parser.add_argument('--model_name', type=str, default= '/mnist6_q8.pth', help='load model')
parser.add_argument('--backend', type=str, default= 'ibmq_montreal', help='load model')
