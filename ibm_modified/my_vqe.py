...
class VQE(VariationalAlgorithm, MinimumEigensolver):
    ...
    def construct_circuit2(
        ...
        weights = []
        ops = []
        circ = self.ansatz.assign_parameters(parameter)

        for op in operator:
            ss = str(op).split('*')
            weights.append(float(ss[0]))
            op = ss[1].strip()
            ops.append(op)
       
        for op in ops:
            # print(op)
            meas = QuantumCircuit(operator.num_qubits)
            # print(len(op))
            ...
            circ_temp  = circ.compose(meas)
            circ_temp.measure_all()
            circuits.append(circ_temp)        

        return weights,ops,circuits
    ...
    def compute_minimum_eigenvalue(
        ...
        # print(energy_evaluation(opt_result.x))
    ...
    def get_energy_evaluation_from_param(
        self,
        operator: OperatorBase,
        parameters,
        return_expectation: bool = False,
    ...
    def compute_minimum_cost_function(
        self, operator: OperatorBase, aux_operators: Optional[ListOrDict[OperatorBase]] = None
    ) -> MinimumEigensolverResult:
        ...
        self._eval_count = 0
        cost_function, expectation = self.get_cost_function(
            operator, return_expectation=True
        )
        ...
        if callable(self.optimizer):
            opt_result = self.optimizer(  # pylint: disable=not-callable
                fun=cost_function, x0=initial_point, jac=gradient, bounds=bounds
            )
        else:
            opt_result = self.optimizer.minimize(
                fun=cost_function, x0=initial_point, jac=gradient, bounds=bounds
        ...
        eigenvalue = energy_evaluation(opt_result.x)
        ...
        result.eigenvalue = eigenvalue + 0j
        result.eigenstate = self._get_eigenstate(result.optimal_parameters)
    ...
    def get_cost_function(
        ...
        def cost_function(parameters):
            ...
            vqe_loss = means if len(means) > 1 else means[0]
            mask = np.zeros(38)
            mask[8:15]=1
            mask[23:30]=1
            # mask[38:45]=1
            base =np.round(parameters/np.pi)*np.pi
            regular_loss = np.sum(np.abs(parameters-base)*mask) 
            # print(regular_loss)
            # sys.exit(0)
            loss = vqe_loss + regular_loss*0.01

            return loss

        if return_expectation:
            return cost_function, expectation
