import enum
import warnings
from math import pi, erf,sqrt  # pylint: disable=no-name-in-module
...
# @staticmethod
def rescale_cr_fix_area(inpulse, stretch_ratio: float, sample_mult: int = 16) -> int:
    ...
    stretch_amplitude = params["amp"] * ori_norm_area / stretch_norm_area
...
def get_stretch_ratio(stretch_ratio,theta):
    mindif = 20
    proper_stretch_ratio = 1
    if isinstance(stretch_ratio,dict):
        for key,val in stretch_ratio.items():
            dif = abs(abs(theta)- float(key))
            if dif < mindif:
                mindif = dif
                proper_stretch_ratio = val
    else:
        proper_stretch_ratio = stretch_ratio
    return proper_stretch_ratio
...
class RZXStretchCalibrationBuilder2(CalibrationBuilder):
    """
    ...
    def __init__(
        ...
        self.stretch_ratio = dict()

        self.coupling_map = instruction_schedule_map.qubits_with_instruction('cx')
        # print(self.coupling_map)
        # sys.exit(0)
        if isinstance(stretch_ratio,(float, int)):
            for qubits in self.coupling_map:
                self.stretch_ratio[str([qubits[0],qubits[1]])] = stretch_ratio
        else:
            for qubits in self.coupling_map:
                self.stretch_ratio[str([qubits[0],qubits[1]])] = 1

        if isinstance(stretch_ratio,(dict)):
            for qubits,value in stretch_ratio.items():
                self.stretch_ratio[qubits] = value
    ...
    @staticmethod
    @builder.macro
    def rescale_cr_amplitude_inst(instruction: Play, theta: float, stretch_ratio: float, sample_mult: int = 16) -> int:
        """A builder macro to play stretched pulse.
        ...
        stretched_pulse = GaussianSquare(**params)
        stretched_pulse,round_duration = rescale_cr_fix_area(stretched_pulse,stretch_ratio, sample_mult)
        builder.play(stretched_pulse, instruction.channel)
        return round_duration
    ...
    def get_calibration(self, node_op: CircuitInst, qubits: List) -> Union[Schedule, ScheduleBlock]:
        ...
        stretch_ratio = self.stretch_ratio[str(qubits)]
        if isinstance(stretch_ratio,dict):
            stretch_ratio = get_stretch_ratio(stretch_ratio,theta)
            ...
            with builder.build(
                ...
                for cr_tone, comp_tone in zip(cr_tones, comp_tones):
                    with builder.align_left():
                        self.rescale_cr_amplitude_inst(cr_tone, theta,stretch_ratio)
                        self.rescale_cr_amplitude_inst(comp_tone, theta,stretch_ratio)
                    builder.call(xgate)
...
class RZXStretchCalibrationBuilderNoEcho2(RZXStretchCalibrationBuilder2):
    """
    Creates calibrations for RZXGate(theta) by stretching and compressing
    ...
    def get_calibration(self, node_op: CircuitInst, qubits: List) -> Union[Schedule, ScheduleBlock]:
        ...
        # print(stretch_ratio)
        # print('theta: {} ,final stretch_ratio:{}'.format(2*theta ,stretch_ratio))
        # sys.exit(0)

        # key = str([0, 1])
        stretch_ratio = self.stretch_ratio[str(qubits)]
        if isinstance(stretch_ratio,dict):
            stretch_ratio = get_stretch_ratio(stretch_ratio,2*theta)
        ...
        if comp_tones[0].channel.index == qubits[1]:
            with builder.build(default_alignment="left", name="rzx(%.3f)" % theta) as rzx_theta:
                stretched_dur = self.rescale_cr_amplitude_inst(cr_tones[0], 2 * theta,stretch_ratio)
                self.rescale_cr_amplitude_inst(comp_tones[0], 2 * theta,stretch_ratio)
