...
class RZXCalibrationBuilder(CalibrationBuilder):
    ...
    @staticmethod
    @builder.macro
    def rescale_cr_fix_area(instruction: Play, amplitude_ratio: float, sample_mult: int = 16) -> int:
        """A builder macro to play stretched pulse.
        ...
        try:
            ...
            raise QiskitError("Target amplitude_ratio is not assigned.") from ex
        ...
        # Area is normalized by amplitude.
        # This makes widths robust to the rounding error.
        norm_risefall_area = params["sigma"] * np.sqrt(2 * pi) * erf(risefall_sigma_ratio)
        full_area =(params["width"] + norm_risefall_area)* params["amp"]
        new_amp = params["amp"] * amplitude_ratio
        new_norm_risefall_area = full_area * amplitude_ratio
        new_width = (full_area - new_norm_risefall_area)/new_amp
        if new_width >=0:
            params["amp"] = new_amp
            params["width"] = new_width
        else:
            print("wrong amplitude_ratio, please make it bigger.")
            
        round_duration = (
            round((new_width + risefall_sigma_ratio * params["sigma"]) / sample_mult) * sample_mult
        )
        params["duration"] = round_duration
        stretched_pulse = GaussianSquare(**params)
        builder.play(stretched_pulse, instruction.channel)
